...
3. The thin install replaces reading the full core: 10 KB (`KAIF.md`, 10 207 bytes) instead of
   339 KB (`KAIF-FULL.md`, 346 615 bytes) — ×34 less reading. Cognitive writing shrinks ×66
   (measured at the 1.5 field certification,
   [homework 02](homeworks/02_DONE_field_test_thin_install_on_weak_llm.md)).
...
`/nightloop` (морning report), `/autoloop` (a long series), or `/guarded-loop` (under an external
...
framework/                            the canonical universal templates (the payload)
  ...
  adapters/                           agent-system adapters (five systems)
...
3. Тонкая установка заменяет чтение полного ядра: 10 КБ (`KAIF.md`, 10 207 байт) вместо 339 КБ
   (`KAIF-FULL.md`, 346 615 байт) — чтения меньше в 34 раза. Когнитивное письмо сокращается в
   66 раз (измерено при полевой сертификации 1.5,
   [домашка 02](homeworks/02_DONE_field_test_thin_install_on_weak_llm.md)).
...
framework/                            канонические универсальные шаблоны (полезная нагрузка)
  ...
  adapters/                           адаптеры агентских систем (пять систем)
  templates/_owner-voice-template.md  скелет портрета голоса владельца (едет в .kaif/)
  templates/languages/                10 языковых пакетов (документы владельца + алиасы навыков)
...
tools/                                build-framework.mjs · check-framework.mjs · sandbox-suite.mjs (полигон)
                                      · module-map-lib.mjs · build-diagrams.mjs · readme-pdf.mjs · commit.mjs · kaif.mjs
AGENT_GUIDE.md · STATUS.md · …        обвязка самообвязки: фреймворк, применённый к самому себе
