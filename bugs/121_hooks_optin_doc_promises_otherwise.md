...
- **Страж F9 — исполнение, а не токен:** `s14` извлекает оба блока из РАЗВЁРНУТОГО README и исполняет каждую строку в её
  ...
  **Граница стража, найденная судьёй сессии 67 (F1, 2026-09-18 14:37 +03:00) и починенная до пуша:** закрытый stdin
  отдаёт хуку EOF мгновенно, поэтому строка README, потерявшая свой пустой stdin (`< /dev/null`, `'' |`), в скрытом
  запуске НЕ зависает и не краснеет — мутант судьи проходил 87/87; комментарий свода утверждал обратное. Грабля «хук,
  запущенный руками без stdin, ждёт терминал» держится теперь СТАТИЧЕСКИМ ассертом по тексту строк (каждая строка каждого
  блока даёт хуку stdin перенаправлением или конвейером; пустой набор строк — красный), мутант М5 краснит ровно его.
  Тем же заходом снято недостающее наблюдение о BOM (F9): скрытый запуск BOM не ставит вовсе, грабля живёт при вводе
  консоли в UTF-8 — отчёт, прогоны 14–15.
...
хука 1); `FORBIDDEN` — красный на README ревизии `95c7883`, зелёный на дереве. После починки F1 (2026-09-18 14:44–14:45
+03:00): 88 ассертов зелёных; сборка до фикса — 14 красных (прежние 13 + статический); пять мутантов — 2 · 1 · 1 · 1 · 1.
