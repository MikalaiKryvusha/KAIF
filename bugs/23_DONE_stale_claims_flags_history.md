...
`scanStaleClaims` не сканирует директории знаний и EXPERIENCE/PROJECT_HISTORY; в STATUS.md
пропускает строки «предыдущ/previous». Стражи s07/T4 (3 шт.) красные до / зелёные после; ревью Q5
подтвердило: README корня и plans/ сканируются по-прежнему.

## Decisions made without the owner

- Из трёх вариантов фикса выбраны исключения (форма), а НЕ разделение вывода на «протухло/история»
  — по Оккаму; если поле попросит секцию «вероятно история», добавить дёшево.
- `plans/` сознательно НЕ исключены: план — утверждение о будущем, не летопись.
- `PROJECT_HISTORY.md` добавлен в исключения авансом (эпик H введёт документ — скан не должен
  будет меняться).
