<!--
  This file is the SOURCE of the Slim KAIF artifact.
  The root KAIF-SLIM.md is GENERATED from this file by tools/build-framework.mjs
  (only the {{VERSION}} / {{RELEASED}} stamps are substituted). Edit THIS file,
  then re-run the build tool. Do NOT edit the root KAIF-SLIM.md by hand.

  Slim KAIF is NOT an installer. Unlike KAIF.md, it does not unpack into 37 files.
  It IS the framework, in place: one file the agent reads and works by. Keep it lean.
-->
# KAIF — Slim · one file, no unpacking · v{{VERSION}}
