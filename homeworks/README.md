...
описывает задачу конкретными шагами для человека и собирает его наблюдения и результаты. По одному
`NN_<имя>.md`.
...
**Вовне:** (`AGENT_GUIDE.md` → «Шапка-мета документов»). Получив ответ человека, учти результаты и
пометь файл `DONE` (`git mv NN_x.md NN_DONE_x.md`).

**Домашка класса «вкус»** (критерий приёмки — прилагательное восприятия; `AGENT_GUIDE.md` → «Класс
"вкус"»): агент отдаёт человеку АРТЕФАКТ восприятия, не ссылку и не чужой бенчмарк; все кандидаты
на ОДНОМ материале, вслепую, расшифровка рядом. Два постоянных поля в каждом таком документе:
**«Что уже можно слушать/смотреть прямо сейчас»** (пути к артефактам) и **«Вынесенные вердикты»**
(решения владельца дословно — вердикт есть канон и не спрашивается дважды).

> Директория заведена в версии 1.1; наполняется по мере появления заданий.
