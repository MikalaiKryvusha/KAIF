...
> **Создан:** 2026-08-28 (передан владельцем; текст подготовлен ChatGPT — внешний разведдок-справочник).
> **Родитель:** `ideas/26` § «Запрос владельца…», п. 3. **Статус:** 📚 справочник эпика TD —
> исполняется НЕ построчно, а Парето-разрезом по `plans/78` (детальный разрез — на открытии
> эпика TD). **Вовне:** —.
