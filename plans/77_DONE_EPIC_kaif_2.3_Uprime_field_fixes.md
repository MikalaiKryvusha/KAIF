...
> **Статус:** ✅ ЭПИК ЗАКРЫТ 2026-08-21 03:29 +03:00 — все шесть фаз; лёгкий судья по критериям
> 1–6 зелёный (секция «Судья эпика»); таблица исходов — 9/9 issues + все взятые R2-вхождения;
> черновики ответов во все девять issues (отправка — слово владельца). Вехи фаз:
> нулёвка U′0 пройдена 2026-08-21 01:22 +03:00 (issues #3–#22
...
> живой v2.2 сходится). ✅ Ответы ОТПРАВЛЕНЫ и все девять issues (#19 #15 #16 #8 #10 #3 #4 #6
> #20) закрыты 2026-08-21 19:10 +03:00 (слово владельца — №80 + чат 2026-08-21; подпись
> агента-автора по канону issue #15; «for the upcoming 2.3» заменено фактом вышедшего релиза
> v2.3; семь полевых отчётов #5 #7 #9 #11 #12 #13 #17 оставлены открытыми — не дефекты).
...
### U′5 — заморозка восьми пакетов и закрытие эпика (№56/№57; 92.2–92.5) — ✅ закрыта 2026-08-21 03:29 +03:00

- [x] 1. Объявление заморозки по формуле №56 (версия 2.2 · состояние полное, побайтно
      запинено · причина «тяжело в поддержке, пока не окупается» · оживление по запросу
      сообщества) — в README ОБЕИХ половин (§7.3), в поставке (Reference §7.4 переписан:
      два живых пакета en/ru, восемь frozen) и в САМОЙ установке: `logPackHonesty` печатает
      строку статуса при развёртывании замороженным пакетом (владелец узнаёт из прогона, не
      из документации). Пакеты в бандле, НЕ удалены (162 блока стабильны). Свод s07/Freeze:
      de несёт строку и ставится с exit 0, ru строку НЕ несёт; красный доказан против dist
      до пересборки.
- [x] 2. Новый страж `tools/lang-packs-guard.mjs` + лок `tools/lang-packs.lock.json`
      (8 пакетов × 9 файлов побайтно EOL-норм — CRLF-чекаут не красит, класс bug 12; живой
      ru — состав поимённо + пол объёма 40 % против EN-исходника; 35 пинов EN-описаний
      навыков). Закрывает поимённо: 92.5 — ростер пакетов ИЗ ЛОКА, не с диска; 92.3 —
      потеря/подмена/посторонний файл в любом пакете; 92.4 — мутация байта замороженного и
      выпотрошенный живой перевод; 92.2 — переписанное EN-описание при несверенных
      ru-алиасах (пины двигает `--update-pins` после сверки глазами; замороженные sha —
      только `--write-lock` словом владельца). `--selftest`: ШЕСТЬ мутаций красные на
      фикстурах, чистая молчит, дерево не тронуто. В реестре пар (→ `/end-chat`), в шаге 4
      `/release`, в таблице инструментов.
- [x] 3. Таблица исходов — секцией ниже: 9/9 issues закрыты фиксом с черновиком ответа
      (греп `→ issue #` по этому плану = 9); взятые R2-вхождения — все с исходами.
- [x] 4. Замер лесов (проба git ls-tree против `49c32c3`, старт U′): инструменты
      `tools/*.mjs` 29 → 31 (+2 стража: release-body-guard, lang-packs-guard), строки
      10 425 → 10 844; своды полигона 14 → 14 файлов (строки 3 592 → 3 915 — ассерты в
      существующих). ЧЕСТНО: масса лесов ВЫРОСЛА (+742 строки) — эпик U′ по построению
      добавляет стражей на оплаченные полем классы (критерий эпика 3: каждый фикс со
      сводом), и каждый доказан красным, охват = заявленному; СОКРАЩЕНИЕ массы — мандат
      эпика T 2.4 («один движок + правила-данные», №73), оба новых стража — первые
      кандидаты в его правила-данные. Число сводов не выросло. Уроки: EXP-0085 (libuv/
      дренаж + пайп-артефакт замера), EXP-0086 (одно пространство суждения) — оба с полем
      механизации; «Решения без владельца» дополнены; **детализация фазы W — нулёвкой W0
      на её открытии** (реестр суда пересобрать по факту HEAD).
- **Гейт U′5 = критерии эпика 1–6: ✅ [TESTED: 2026-08-21 · см. «Судья эпика» ниже]**

## Таблица исходов (критерии эпика 1–2)

| Сигнал | Исход | Улика |
|---|---|---|
| issue #19 | починено + страж | U′1: `review.mjs` `if(st||ac)` + P7 три состояния; блок 8в verify-contour, мутация 147/2→149/0; черновик ↓ |
| issue #15 | починено (канон обоих слоёв) | U′1: `/report-bug` ветка фреймворка — автономно с подписью агента; черновик ↓ |
| issue #16 | починено + страж | U′2: сплайс `package.json`; s01/S5a+S5b, красный 5; `7bb1753`; черновик ↓ |
| issue #8 | починено + страж | U′2: переход anonymous→origin на маркере + отказ до записи; s04/S15a–c, красный 4; `7bb1753`; черновик ↓ |
| issue #10 | две оси починены + страж; ось «Δ называет модуль» — 2.4 (названо в черновике) | U′2: резолв голого URL + формы отказа + libuv-класс `dieSoft` (пробой 5/5→0); s09/D6, красный из `7bb1753`; `7d9258c` |
| issue #3 | починено + страж | U′3: сфера — будущий член списка placeholders; s01/S6; `012a1a0`; черновик ↓ |
| issue #4 | починено + страж | U′3: пункт owner-voice ДО goal-plan с исполняющим гейтом; s01/S6 + фикстуры s04/s07; `012a1a0`; черновик ↓ |
| issue #6 | починено | U′3: язык по АУДИТОРИИ, оба слоя; s01/S6 три ассерта; `012a1a0`; черновик ↓ |
| issue #20 | починено + страж | U′4: одно пространство суждения + отказ на ненайденном заголовке; контракт issue дословно зелёный, мутации A/B; `3fa0e9f`; черновик ↓ |
| 92.1 | починено + страж | U′2: `checkedLang` (слово→подсказка, не-код→отказ, маркер лечится); s09/D7, красный 4 |
| 92.2 | починено + страж | U′5: пины EN-описаний (35) в lang-packs-guard; селфтест «ПЕРЕПИСАНО» красный |
| 92.3 | починено + страж | U′5: состав пакетов поимённо + sha; селфтесты «файл потерян»/«посторонний» красные |
| 92.4 | починено + страж | U′5: побайтный пин замороженных + пол 40 % живого ru; селфтесты красные |
| 92.5 | починено + страж | U′5: ростер пакетов ИЗ ЛОКА; селфтест «ПОТЕРЯН целиком» красный |
| 98.1 | починено | U′4: шаг 4 `/release` — пять реальных команд истока; `9f6812e` |
| 98.2 (класс стража) | построен + доказан | U′4: release-body-guard, селфтест красный без сети, живой v2.2 сходится (само расхождение v2.2 — STALE по триажу, снято до эпика) |
| 98.3 | точечно (маркер по наблюдению); остальное — 2.4 | U′4: readme-pdf.mjs [TESTED] по живому прогону |
| 99.1 | починено + страж | U′2: `translatedOnDisk` считает диск; s04/S16, красный «8 при 6» |
| 99.2 | починено + страж | U′2: анти-фильтр в logPackHonesty; s04/S16 «bodies==aliased», красный 35≠31 |
| 99.3 | починено + страж | U′2: отказ лоадера на неизвестном канале; s09/D6, красный из `7bb1753` |
| §12.3 квитанция-момент | закрыто наследием T8, подтверждено наблюдением | s03: ISO_MOMENT_RE на date/history.date/verifiedAt (S9:77, S9:82, S12:238) |
| crash-safety `update` | построено + страж | U′2: журнал + `resume`, Reference §12.4; s02/S9-crash, красный 9; `555ac5e` |
| Заморозка №56/№57 | исполнена + страж | U′5: три носителя объявления + lang-packs-guard + лок; s07/Freeze |

## Судья эпика (лёгкий, №48) — критерии приёмки 1–6

1. ✅ Девять issues закрыты фиксом, черновики ответов — все девять секцией ниже (греп
   `**→ issue #` = 9); отправка — только слово владельца.
2. ✅ Взятые R2-вхождения несут исходы (таблица выше; «отклонено владельцем» не понадобилось —
   нулёвка ничего не выносила).
3. ✅ Каждый фикс машинерии — свод, красный на сломанной версии (5+4+D6+D7-4+S16-2+9 красных по
   фазам, красные — против реально сломанных версий, не мутаций ради галочки); `npm run
   test:core` — «all 14 suites green» на закрытии каждой фазы и финально.
4. ✅ Контур тронут только в U′1 — полный `verify-contour` в живом браузере 149/0 там же; страж-
   кейс «заполнен только комментарий → сохранён» — блок 8в.
5. ✅ Заморозка по формуле №56 в README обеих половин + Reference §7.4 + строка установки; оси
   92.2–92.5 знают заморозку поимённо (лок + шесть красных селфтестов).
6. ✅ Оба слоя двигались вместе (payload + обвязка); пересборка чистая (§9.11 поймал и вернул
   кириллицу в U′3 — перефразировано); дерево зелёное: полигон 14/14 · counters 50 зеркал ·
   doc-header 0 · experience-lint 0 новых · README.pdf перегенерирован.

Вердикт лёгкого судьи: **эпик U′ закрыт по своим критериям**; полный адверсарный суд ВСЕЙ
версии — фаза W (`plans/75`), как назначено №74/№77.
...
8. **U′4: #20 — раздел 0 едет ЦЕЛИКОМ, включая канон-текст двери с алиасами** — по замыслу
   владельца и репортёра («раздел старше всех чисел», «правила едут целиком по решению №60»);
   сужение публикации было бы моим самовольным решением против «правила должны доезжать».
   Спан «project A Space Rating» (4 слова после алиаса) схлопнут честно — внесение настоящего
   имени в allowSpans опубликовало бы его (граница issue).
9. **U′4: 98.3 взят ТОЧЕЧНО** (маркер readme-pdf по живому наблюдению этой сессии — правило
   TESTING «улики против маркера»); остальное 98.3 остаётся скоупом 2.4, как назначил триаж.
10. **U′5: механика заморозки** — лок с побайтными (EOL-норм) sha восьми пакетов; пол объёма
    живого ru — 40 % строк EN-исходника (консервативный: честные переводы ходят у 100 %);
    92.2 закрыт ПИНАМИ EN-описаний (35), а не пином значений алиасов — протухание ловится в
    момент, когда EN-описание двигается, и лечится сверкой глазами + `--update-pins`; honesty-
    строка заморозки живёт В УСТАНОВКЕ (владелец узнаёт из прогона), список FROZEN_PACKS зашит
    в ядро 2.3 как политика версии.
11. **U′5: замер лесов напечатан ЧЕСТНО с ростом** (+2 инструмента, +742 строки): критерий
    эпика 3 требует свод на каждый фикс, и рост — цена оплаченных полем классов; сокращение
    массы — мандат эпика T 2.4, оба новых стража — первые кандидаты в его правила-данные.

## Черновики ответов в issues (U′2; отправка — ТОЛЬКО `send-outbound` со словом владельца)

**→ issue #19 (контур теряет замечание к артефакту без статуса):**

> Fixed for the upcoming 2.3, both carriers of the class. The artifact branch of the origin's
> own contour (`tools/review.mjs` collect(), the `if(st)` you named) now records a comment WITH
> OR WITHOUT a chosen status — and a comment alone does not open the approval gate (only
> `approved` does, by construction). The payload contract of `/owner-reviews` got the same class
> closed as its P7 clause, widened: a drawn field owes the human its content; a snapshot has
> THREE states, and "options rejected, here is the direction" is the STRONGEST one — it reads as
> a work stop, never as silence; a general comment is not a question's comment. The guard is a
> live-click case in the contour's acceptance (block 8в of `verify-contour`), proven red by
> re-introducing the `if(st)` mutation: 147 green / 2 red before the fix, 149/0 after. The
> owner's words are the one thing a review contour exists to keep — thank you for catching the
> branch that dropped them.

**→ issue #15 (транспорт ≠ авторство — сигналы агентов стоят в очереди к человеку):**

> Fixed for the upcoming 2.3, as the standing rule you quoted ("это КАНОН"): the framework
> branch of `/report-bug` now sends an agent's defect signal about KAIF itself to the origin
> AUTONOMOUSLY, signed by the agent-author — transport is not authorship, and a bug ticket is
> the agent's own text answering for its own observations. The send-gate of `/owner-reviews`
> stays exactly where it belongs: foreign repositories and any statement made IN THE OWNER'S
> NAME. Both axes are named in both skills, both layers, so a weak session meets the rule at the
> decision point instead of deriving it. Your own tickets — filed from the owner's account with
> the authorship line inside — were the working model for the wording.
