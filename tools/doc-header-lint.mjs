...
// Формы заявления сняты С ЖИВЫХ документов, а не придуманы (сессия 67, 2026-09-18). Прежняя редакция знала только
// `✅ ЗАКРЫТ` — слово вплотную к знаку, верхним регистром — и метку `**Status:**` в САМОМ НАЧАЛЕ файла (якорь без флага
// `m`, то есть мёртвая ветка). Каждый план эпика версии 2.7 закрывался иначе: `> **Статус:** ✅ **ЗАКРЫТ 2026-…`,
// `✅ **ЭПИК ЗАКРЫТ …`, `✅ закрыт …`, `✅ исполнен …` — и ось их не видела: в один день шесть закрытых планов несли
// тело «до» (метрика «0 из 1», пустые клетки шагов) при зелёном линте. Замер до правки — проба на копии: ось 0 находок,
// расширенная — 8 (шесть исторических документов, снятая редакция плана, один законно ждущий пункт). Кириллица — явными
// формами, без флага `i` (EXP-0032).
const CLOSED_CLAIM_RE = /(?:✅\s*\**\s*(?:ЭПИК\s+)?(?:DONE|ЗАКРЫТ[АО]?|[Зз]акрыт[ао]?)(?![а-яё])|\*\*(?:Status|Статус):\*\*\s*✅)/u;
...
function selftest() {
  ...
  // Ось «закрытие против тела» (bugs/76). До сессии 67 у оси не было НИ ОДНОЙ фикстуры, и её примета знала форму,
  // которой документы проекта не пишут, — страж, не краснеющий в собственном селфтесте, ничего не доказывал. Четыре
  // красных — четыре ЖИВЫЕ формы закрытия (сняты с планов версии 2.7 и с исторических планов), две молчащих — все
  // законные двери сразу и открытый документ с пустыми клетками.
  const statusBody = (title, status, tail) => `# ${title}\n\n> **Создан:** 2026-08-07. **Родитель:** \`plans/30\`. **Статус:** ${status} **Вовне:** —.\n` +
    '\n## Вектор цели\n\nДостичь X, наблюдаемого прогоном Y (Achieve).\n' +
    '\n## Критерии приёмки (готово, когда)\n\n1. Сборка проходит за ≤ 60 с на референс-машине CI.\n' + tail;
  const EMPTY_STEP = '\n## Шаги\n\n- [x] шаг первый, отмеченный.\n- [ ] шаг второй — клетка пуста под закрытым статусом.\n';
  writeFileSync(join(fx, 'plans', '41_epic29_closed_bold.md'),
    statusBody('План 41 — закрыт жирной вехой', '✅ **ЗАКРЫТ 2026-08-09 10:00 +03:00** живым наблюдением.', EMPTY_STEP));
  writeFileSync(join(fx, 'plans', '42_epic29_closed_lower.md'),
    statusBody('План 42 — закрыт строчными', '✅ закрыт 2026-08-09 10:00 +03:00 (интегратор).', EMPTY_STEP));
  writeFileSync(join(fx, 'plans', '43_epic29_closed_epic.md'),
    statusBody('План 43 — эпик закрыт', '✅ **ЭПИК ЗАКРЫТ 2026-08-09 10:00 +03:00** интеграцией передачи.', EMPTY_STEP));
  writeFileSync(join(fx, 'plans', '44_epic29_closed_label.md'),
    statusBody('План 44 — исполнен, слова «закрыт» нет', '✅ исполнен — релиз отгружен 2026-08-09 10:00 +03:00.', EMPTY_STEP));
  //   ожидаем: 4 красных — по одной на документ
  // Пятая красная — заявление БЕЗ метки `**Статус:**` (находка 3 лёгкого судьи дельты f4915af: четыре фикстуры выше видны
  // одной веткой метки, и первая ветка константы — жирное · строчное · «ЭПИК» — оставалась недоказанной).
  writeFileSync(join(fx, 'plans', '47_epic29_closed_no_label.md'),
    statusBody('План 47 — закрытие названо вехой, а не меткой статуса', 'в архиве.',
      '\nВеха: ✅ **ЭПИК ЗАКРЫТ 2026-08-09 10:00 +03:00** интеграцией передачи.\n' + EMPTY_STEP));
  //   ожидаем: 1 красная — её видит ТОЛЬКО первая ветка константы
  writeFileSync(join(fx, 'plans', '45_epic29_closed_doors.md'),
    statusBody('План 45 — закрыт, все пустые клетки законны', '✅ закрыт 2026-08-09 10:00 +03:00.',
      '\n## Прежняя редакция шагов <!-- closed-ok: редакция снята словом владельца и не исполнялась -->\n\n- [ ] шаг отменённой постановки.\n' +
      '\n## Пул автономного беклога\n\n- [ ] хвост, который и есть содержание списка-хранилища.\n' +
      '\n## Шаги\n\n- [x] шаг отмеченный.\n- [ ] отправка ответа — после релиза. <!-- closed-ok: ждёт релиза словом владельца -->\n'));
  writeFileSync(join(fx, 'plans', '46_epic29_open_boxes.md'),
    statusBody('План 46 — в работе, пустые клетки законны', 'в работе.', EMPTY_STEP));
  //   ожидаем: 0 — три двери (изъятие на заголовке · список-хранилище · изъятие на строке) и незакрытый документ
  const expected = 24; // 4 + 1 + 2 + 1 + 1 + 2 + 1 + 2 (имена plans/) + 4 (метки T8) + 1 (PENDING) + 5 (закрытие против тела)
