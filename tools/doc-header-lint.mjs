...
// расширенная — 7 (пять исторических документов, снятая редакция плана, один законно ждущий пункт). Кириллица — явными
...
function selftest() {
  ...
  const expected = 23; // 4 + 1 + 2 + 1 + 1 + 2 + 1 + 2 (имена plans/) + 4 (метки T8) + 1 (PENDING) + 4 (закрытие против тела)
