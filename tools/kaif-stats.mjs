...
const HUMAN = {
  // Цена гамбургера — ЧИСЛО ВЛАДЕЛЬЦА (его слово, 2026-08-09): «везде, где доллары, пересчитать в
  // бургеры по 5$. Эта колонка не для комментариев, а для сравнения с бытом» · «для наглядности и
  // представления масштаба метрик обывателем». Прежнее значение 5,91 — средняя цена Big Mac в США
  // 2026 (bigmacindex, 13 500+ ресторанов) — было ТОЧНЕЕ и хуже: круглая пятёрка делит в уме, а
  // бытовая мера существует ровно для того, чтобы её делили в уме. Точность здесь не свойство
  // метрики, а помеха её назначению.
  burgerUsd: 5,
...
if (all) {
  ...
  // ГРАНИЦА БЕРЁТСЯ ПОСЛЕДНИМ КОММИТОМ ДО МОМЕНТА, а не первым ПОСЛЕ него.
  // Прежняя редакция звала `git log --since=<дата> --reverse` и брала первый элемент — и он
  // приходил НЕ самым ранним коммитом окна: между 00:00 и 19:42 седьмого августа лежали 57
  // коммитов, которые в выборку не попали, поэтому окно молча начиналось на девятнадцать часов
  // позже и теряло почти весь первый день работы. Витрина из-за этого показала объём МЕНЬШЕ
  // прежнего замера при БОЛЕЕ ДЛИННОМ окне — арифметически невозможное сочетание, и заметил
  // его владелец, а не проверка. `rev-list -1 --before` возвращает ровно одну ревизию —
  // последнюю до границы, — и она и есть база диффа: всё, что после неё, вошло в окно.
  const base = git('rev-list', '-1', `--before=${sinceISO}`, 'HEAD').trim();
  if (!base) {
    console.error(`до ${sinceISO} нет коммитов — граница окна не определяется`);
    process.exit(1);
  }
  from = base;
} else {
...
if (args.includes('--json')) {
  ...
  // Разряды — ПО СТАНДАРТУ ЯЗЫКА, а не по одному правилу на обе половины (слово владельца
  // 2026-08-09: «используем для русского неразрывный пробел»). По ГОСТу русское число делит
  // разряды пробелом, а запятая занята дробной частью: `123 456,78`. В английской половине
  // наоборот: `123,456.78`. Отчёт этого инструмента русский, поэтому здесь `ru-RU` — он и даёт
  // НЕРАЗРЫВНЫЙ пробел (обычный сломал бы число переносом строки).
  // ⚠️ Для АНГЛИЙСКОЙ половины витрины числа этого отчёта переписываются с запятой — переводить
  // формат обязан тот, кто переносит число, и это сказано здесь, чтобы он не забыл.
