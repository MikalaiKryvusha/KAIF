...
// bugs/104 (№019): пометка легальна и ВНУТРИ жирного — «| **A (Рекомендация)** |»: скобка
// допускается по обе стороны закрывающих `**`, буква по-прежнему одна, поэтому строка данных
// («| **Antigravity** | …») не проходит. Каждая новая форма этого класса — та же грабля bugs/51:
// строка выглядит вариантом для человека, но не для парсера; класс-детектор шире парсера —
// кандидат 2.4 (судьба в STATUS).
const OPTION_ROW_RE = /^\s*\|\s*\*{0,2}([A-ZА-Я])[.)]?(?:\s*\([^)]*\))?\*{0,2}(?:\s*\([^)]*\))?\s*\|(.+)$/u;
...
function selftest() {
  ...
  // bugs/104 (третья форма класса bugs/51, №019): пометка в скобках легальна и ВНУТРИ жирного —
  // «| **A (Рекомендация)** |». Парсер знал пометку только ПОСЛЕ закрытия жирного, строка
  // выпадала молча, и владелец снова решал без варианта-рекомендации. Обе стороны (EXP-0059):
  // все три записи пометки дают радиокнопки; таблица данных с жирным СЛОВОМ — не развилка.
  const fx104 = '# I\n\n> Status: **🟡 awaiting**\n\n### Q1. Вопрос?\n\n| Вариант | Что означает | Цена |\n|---|---|---|\n| **A (Рекомендация)** | пометка внутри жирного | а |\n| **B** (пометка) | пометка после жирного | б |\n| **C** | голая буква | в |\n\n**Answer:**\n';
  const q104 = parseQuestions(fx104)[0];
  ok(q104.options.length === 3 && q104.options.map((o) => o.letter).join('') === 'ABC',
     'разбор: пометка в скобках ВНУТРИ жирного — вариант доезжает (bugs/104)');
  const fxData = '# I\n\n### Q1. Вопрос?\n\n| Система | Статус |\n|---|---|\n| **Antigravity** | жива |\n| **Basis** | жива |\n\n**Answer:**\n';
  ok(parseQuestions(fxData)[0].options.length === 0,
     'разбор: таблица данных с жирными словами — НЕ развилка (узость приметы сохранена)');
