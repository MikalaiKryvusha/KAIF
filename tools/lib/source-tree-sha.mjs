...
export function sourceTreeSha(repoRoot) {
  ...
  for (const r of rel) { h.update(r); h.update('\0'); h.update(readFileSync(join(root, r))); h.update('\0'); }
...
if (process.argv[1] && process.argv[1].replace(/\\/g, '/').endsWith('/source-tree-sha.mjs') && process.argv.includes('--selftest')) {
  ...
  console.log('✅ source-tree-sha --selftest: fresh on a matching fixture, red on a changed byte, on a field-less manifest and on a missing one');
}
