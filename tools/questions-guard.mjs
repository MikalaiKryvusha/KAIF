...
function selftest() {
  ...
  mut('объявленный свободный вопрос → зелёный', 'маркер questions-guard:open — законный ход',
    ...
    false, '');
  // Фикстура намеренно оставляет вопрос БЕЗ ответа при ЗАКРЫТОМ статусе: так проверяется ровно
  // скоуп новой оси (живые документы) и ничего больше. Проставить ответ значило бы задеть ось
  // разноса I20 — и мутация покраснела бы по ЧУЖОЙ причине, доказав не то (грабля EXP-0057).
  mut('ЗАКРЫТОЕ интервью с вариантами прозой → зелёный', 'история не красится (скоуп — живые документы)',
    () => w('interviews/interview_092_test.md',
      PROSE_Q.replace("**🟡 awaiting the owner's answers**", '**✅ ОТВЕЧЕНО 2026-08-08**')),
    false, '');
