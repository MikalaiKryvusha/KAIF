...
//   G7  — (2.6, issue #50) вердикт о предложении владельца («ломает / не умеет / невозможно /
//          противоречит») без блока Разведка: (запрос · нашёл · замер) рядом — смущение, доставленное
...
//   G11 — (2.7, issue #70) АРХЕОЛОГИЯ живого вопроса: вопрос владельцу — утверждение «этого ещё не решено»,
//         и оно проверяется как утверждение. Живой вопрос документа с датой шапки ≥ `ARCHAEOLOGY_SINCE`
//         без строки `<!-- archaeology: … → N hits · read: … · prior: … -->` перед вариантами — нарушение
//         поимённо, с ГОТОВОЙ командой грепа; аттестация с N > 0 и `prior: none` — нарушение того же класса;
//         исключение — `<!-- archaeology: n/a — причина -->`. Разбор — ЯДРОМ поставки (`archaeology()`), один
//         на дверь показа и стража: две правды об одном вопросе класс уже стоил проекту (пилот 008).
//
...
import { parseQuestions, docStatus, archaeology, ARCHAEOLOGY_SINCE } from './lib/review-core.mjs';
...
// G7 (2.6, issue #50 — слово владельца: «если я тебе что-то предлагаю, и тебя это смущает… это тебе
// прямой знак, что нужно идти искать в интернете, что я имел в виду, или у меня спросить»): ВЕРДИКТ о
// предложении владельца — строка, где рядом стоят предмет (владел-/owner/предложен-/proposal) и слово
// вердикта — без блока разведки (`Разведка:`/`Recon:` с `запрос`/`нашёл`/`замер` или query/found/
// measurement) в окне строк рядом. Цитаты «…»/"…"/`…` снимаются ДО проверки: слова владельца
// «ничего не ломает!» — цитата, не вердикт агента. Законное исключение — объявленное, с причиной.
// `\b` в JS — граница ASCII-слова и внутри кириллицы не срабатывает никогда (селфтест-каверза первого
// прогона: `\bломает\b` не матчился) — кириллические слова ограничены лукараундами по \p{L}.
const VERDICT_RE = /(?:(?<!\p{L})ломает(?!\p{L})|не умеет|невозможно|противоречит|\bbreaks?\b|cannot be done|\bimpossible\b|contradicts)/iu;
const SUBJECT_RE = /(?:владел\p{L}*|\bowner'?s?\b|предложен\p{L}*|\bproposal\b)/iu;
...
// G11 (2.7, issue #70 — аудит Unliminium: «13 вопросов принесены владельцу, хотя его прошлые ответы, GOAL.md или
// прогон стенда их уже решили; один — через 44 дня после ответа»; слово владельца того проекта дословно: «Ты задаёшь
// мне вопросы — совершенно не ознакомившись с историей решений… Мы уже обсуждали. Ищи»): АРХЕОЛОГИЯ живого вопроса.
// Ось G6 судит ФОРМУ вопроса, G8 — его СЛОВАРЬ; эта судит УТВЕРЖДЕНИЕ, которое несёт сам факт вопроса. Разбор —
// не свой: `archaeology()` ядра поставки (`framework/tools/contour/core.mjs` через шим `lib/review-core.mjs`), то же
// решение, что у двери показа, — дубль парсера уже разошёлся с ядром один раз (пилот 008).
//
// @guard questions-guard-G11
// THREAT:         вопрос владельцу, который его прошлый ответ, GOAL.md или план уже решили, — владелец отвечает
//                 второй раз (#70: тринадцать раз одному владельцу, один вопрос — через 44 дня после ответа)
// PROVED-AGAINST: селфтест — четыре мутации: живой вопрос интервью с датой шапки ≥ порога без аттестации → красный;
//                 та же фикстура с аттестацией `→ 0 hits · prior: none` → зелёный; аттестация с `→ 4 hits` и
//                 `prior: none` → красный; та же фикстура с датой шапки до порога → зелёный
// GAP:            синонимы и другой язык ось не ищет — она судит НАЛИЧИЕ и ЧЕСТНОСТЬ аттестации, а не качество
//                 поиска (`N = 0` законно); качество держит охота судьи и глаз владельца
// ON-REAL-PATH:   живой прогон истока 2026-09-18 после AQ3 — интервью корпуса старше порога молчат, №033 отвечено;
//                 функциональный прогон на КОПИИ №033 с датой шапки сегодня — отчёт
//                 testcases/reports/2026-09-18_question-archaeology.md
const ARCHAEOLOGY_HINT = 'node tools/review.mjs <док> --check';
  ...
  for (const p of mdFiles(root)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (g5 && !/^\s*[>|#]/.test(line)) {
        ...
        // G7 (issue #50): вердикт о предложении владельца без разведки рядом — смущение, доставленное
        // как приговор. Окно разведки — RECON_WINDOW_LINES строк вверх и вниз; маркер verdict-ok — исключение.
        if (VERDICT_RE.test(bare) && SUBJECT_RE.test(bare) && !VERDICT_OK_MARK.test(line)) {
          // Кавычки снимаются и по АБЗАЦУ: проза врапается на 100 колонок, и цитата «ваше предложение
          // ломает X» переносится на две строки — построчное снятие её не видит (первый живой прогон
          // покрасил собственный план эпика). Абзац — от пустой строки до пустой строки.
          let a = i, b = i;
          while (a > 0 && lines[a - 1].trim() !== '') a--;
          while (b + 1 < lines.length && lines[b + 1].trim() !== '') b++;
          const paraBare = unwrap(lines.slice(a, b + 1).join('\n')).replace(QUOTED_RE, '');
          const from = Math.max(0, i - RECON_WINDOW_LINES), to = Math.min(lines.length, i + RECON_WINDOW_LINES + 1);
          const hasRecon = lines.slice(from, to).some((l) => RECON_HEAD_RE.test(l));
          if (!hasRecon && VERDICT_RE.test(paraBare)) hits.push({ ...mkHit(root, p, i, line, false), verdict: true });
        }
      }
    }
  }
...
function parseInterview(root, p) {
  ...
  // G11 (#70): археология документа — РАЗБОРОМ ЯДРА поставки; порог и форма аттестации живут там же, где дверь.
  const arch = archaeology(md);
  return { file: name, num, closed: st === 'closed', waiting: st === 'waiting', questions, arch,
...
function scanInterviews(root) {
  ...
  for (const f of files) {
    ...
    // G11 (#70): археология живого вопроса. Единица — ВОПРОС; порог, зона и форма аттестации — в ядре поставки,
    // здесь только адрес и слова. Документ с датой шапки до порога и документ без даты ядро не судит вовсе.
    for (const p of iv.arch.problems) out.unanswerable.push({
      file: iv.file, q: p.id, arch: p, key: `${iv.file}#${p.id}#archaeology` });
...
export function runGuard({ root, baselinePath, writeBaseline = false, log = console.log }) {
  ...
  const violations = [
    ...place.map((h) => ({ ...h, kind: h.emptyReason ? 'маркер с ПУСТОЙ причиной'
      : h.prose ? 'вопрос владельцу ПРОЗОЙ вне interviews/ (G5)'
      : h.verdict ? 'вердикт БЕЗ РАЗВЕДКИ (G7, issue #50)' : 'место вопросов',
      ...(h.verdict ? { text: `${h.text} — о предложении владельца сказано «ломает / не умеет / невозможно / противоречит», `
          + 'а блока Разведка: (запрос · нашёл · замер) рядом нет. Слово владельца: «если я тебе что-то предлагаю, и тебя это '
          + 'смущает… это тебе прямой знак, что нужно идти искать в интернете, что я имел в виду, или у меня спросить». '
          + 'Сначала поиск и замер по его данным, потом вопрос в interviews/ — или объяви исключение: '
          + '<!-- questions-guard:verdict-ok причина -->' } : {}) })),
    ...
    ...iv.unanswerable.map((u) => (u.arch
      ? { ...u, kind: u.arch.kind === 'hits-without-prior'
          ...
          text: u.arch.kind === 'hits-without-prior'
            ...
            : u.arch.kind === 'malformed'
            ? `${u.q}: строка археологии есть, но без «→ N hits» и «prior:» — аттестацией она не является; `
              + 'приведи к форме `<!-- archaeology: <команда> → N hits · read: <файлы | none> · prior: <none | «прошлый ответ …» + адрес> -->`'
            : `${u.q}: живой вопрос без строки археологии (дата шапки ≥ ${ARCHAEOLOGY_SINCE}). Вопрос владельцу — `
              + 'утверждение «этого ещё не решено», и оно проверяется как утверждение (слово владельца полевого проекта: '
              + '«Ты задаёшь мне вопросы — совершенно не ознакомившись с историей решений… Мы уже обсуждали. Ищи»). '
              + `Выполни поиск:  ${u.arch.grep}  — прочитай попадания и впиши между заголовком вопроса и первым вариантом `
              + '`<!-- archaeology: <команда> → N hits · read: <файлы | none> · prior: <none | «прошлый ответ …» + адрес> -->`; '
              + `ту же команду печатает дверь показа (${ARCHAEOLOGY_HINT}); искать нечего (имя, класс «вкус») — объяви `
              + 'исключение: <!-- archaeology: n/a — причина -->' }
      : u.vocab
...
function selftest() {
  ...
  // G7 (2.6, issue #50) — три мутации на фикстуре из инцидента: вердикт без разведки → красный; тот же
  // вердикт с блоком Разведка: в окне → зелёный; вердикт внутри цитаты владельца → зелёный (цитата, не приговор).
  const VERDICT_LINE = 'Одно не исполнено: предложение владельца «role-playing game и RPG одновременно» ломает модель пар тегов — третий тег роняет корпус.\n';
  mut('вердикт «ломает модель» о предложении владельца без разведки (G7, #50) → красный', 'нарушение «вердикт БЕЗ РАЗВЕДКИ»',
    () => w('plans/95_x.md', '# План 95 — тест\n\n> **Создан:** 2026-09-05\n\n' + VERDICT_LINE), true, 'БЕЗ РАЗВЕДКИ');
  mut('тот же вердикт с блоком Разведка: рядом (G7) → зелёный', '0 нарушений: запрос · нашёл · замер стоят в окне',
    () => w('plans/95_x.md', '# План 95 — тест\n\n> **Создан:** 2026-09-05\n\nРазведка:\n- запрос: «РПГ ролевая игра»\n- нашёл: РПГ — русское написание RPG\n- замер: 90 записей каталога несут пару\n\n' + VERDICT_LINE), false, '');
  mut('«ничего не ломает!» в цитате владельца (G7) → зелёный', '0 нарушений: цитата «…» снята до проверки',
    () => w('plans/95_x.md', '# План 95 — тест\n\n> **Создан:** 2026-09-05\n\nОтвет владельца дословно: «ролевая игра и РПГ — ничего не ломает!» — предложение принято.\n'), false, '');

  // G11 (2.7, issue #70) — четыре мутации: живой вопрос документа с датой шапки ≥ порога без аттестации → красный;
  // та же фикстура с аттестацией → зелёный; аттестация с попаданиями и `prior: none` → красный; дата шапки до
  // порога → зелёный (историю поля не переписываем). Фикстура несёт сценарий и таблицу вариантов, иначе мутация
  // краснела бы по ЧУЖИМ осям G6/bugs-62 и доказывала бы не то, что заявлено (грабля EXP-0057).
  const IV_ARCH = (date, attestation) => '# Interview #096\n\n> Status: **🟡 awaiting**\n> Created: ' + date + '\n\n'
    + '### Q1. Как назвать валюту игры?\n\n' + attestation
    + '| Вариант | Что означает |\n|---|---|\n| **A** | кристаллы |\n| **B** | монеты |\n\n' + SCEN + '\n**Answer:**\n';
  const ATT_0 = '<!-- archaeology: grep -rniE "назва|валю|игры" interviews/ GOAL.md MASTER_PLAN.md plans/ → 0 hits · read: none · prior: none -->\n\n';
  mut('живой вопрос интервью от порога без аттестации (G11, #70) → красный', 'нарушение «ВОПРОС БЕЗ АРХЕОЛОГИИ» с напечатанной командой',
