...
// Громкость зова для класса «сообщение» — та же, что у вопроса: писки И голос. Так решил
// владелец: интервью №009, Q3 = B («сообщение тоже зов»); рекомендация агента была A (тише
// вопросов, без голоса) и отвергнута. Поэтому ниже нет отдельной ветки приглушения — сообщение
// проходит ту же цепочку signalCall, что и вопрос, и отличается только ТЕКСТОМ фразы.
...
export function serveContour(root, { docPath = null, batch = false, notice = false }, opts = {}) {
  ...
  // («Нечего записывать»), а кнопки «ОК, прочитано» у обычной страницы нет — полевой отказ
  // 2026-08-21, трижды подряд: «я просто ок нажал, а требует необязательный комментарий».
  // Флаг --notice остаётся явным способом для документов С вопросами; артефактный гейт (outbound
  // draft) под автовывод не попадает — там «прочитано» не исход, нужно одобряю/отклоняю.
  if (docPath && !batch && !notice) {
    try {
      ...
      if (parseQuestions(mdAuto).length === 0 && !(metaAuto && metaAuto.artifacts && metaAuto.artifacts.length)) {
        notice = true;
        log('В документе нет распознанных вопросов и исходящих — показываю классом «' + NOTICE_KIND_LABEL + '» (I37, bugs/105).');
      }
    } catch { /* нечитаемый документ упадёт ниже своим голосом, не здесь */ }
  }
