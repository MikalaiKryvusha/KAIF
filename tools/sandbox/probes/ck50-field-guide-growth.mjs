...
// [TESTED: 2026-09-25 00:08 +03:00 · run over the four 2.7 field deployments on this machine, exit 0, with and without
//  --ref; arrived + own of the two cut deployments equal wc -l (814 + 1115 = 1929, 859 + 520 = 1379); the first run
//  dropped the first directory when --ref was absent (filter excluded index 0) — fixed, re-run; report
...
for (const dir of dirs) {
  ...
  for (const e of edited) {
    ...
    console.log(`   edited ${String(e.n).padStart(4)} (template ${e.t === undefined ? '  -' : String(e.t).padStart(3)})${counted ? `  ${e.n - e.t >= 0 ? '+' : ''}${e.n - e.t}` : '  (not in G)'}  ${e.sig.slice(0, 72)}`);
  }
  for (const e of local) console.log(`   local  ${String(e.n).padStart(4)}                 (not in G)  ${e.sig.slice(0, 72)}`);
  console.log(`   G = ${g >= 0 ? '+' : ''}${g} lines of growth in edited template modules (H1 and "${NOTES_SIGNATURE.slice(3)}" left out)`);
  for (const c of CEILINGS) {
    const t = Math.floor(BUDGET * c);
    const own = Math.round(t * TRANSLATION_LINE_RATIO) + g;
    console.log(`   same shape translated wholesale, template at ${c} × ${BUDGET} = ${t}: ${t} × ${TRANSLATION_LINE_RATIO} + ${g} = ${own} → ${own <= BUDGET ? `passes, ${BUDGET - own} lines to spare` : `STOPS, ${own - BUDGET} over`}`);
  }
}
