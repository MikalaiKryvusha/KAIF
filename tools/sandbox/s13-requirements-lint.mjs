...
// Число предсказанных находок ПЕЧАТАЕТ САМ СЕЛФТЕСТ и держит в своей константе `expected` —
// здесь оно не дублируется прозой (bugs/68, EXP-0025: рукописный счётчик рядом с кодом протухает
// молча). Свод судит ФАКТ: селфтест зелёный, предсказание сошлось, композиция со словарём
// отработала. Зашитое «16» покраснело ровно тогда, когда ось меток честно выросла до 18.
ok(r.code === 0 && /selftest: ok — \d+ предсказанных/.test(r.out) && /композиция со словарём находит стоп-слово/.test(r.out),
  's13 обвязочный линтер — селфтест: предсказание сошлось + композиция со словарём', r.out);
