...
// Прогноз CK5.6 на клонах поля: первое закрытие после 2.8 у всех четырёх развёртываний встаёт на линте авторства без базы (11–151
// унаследованных находок). Навык закрытия обязан назвать ход — записать унаследованный долг один раз, — как он называет его журналу опыта.
ok(/kaif-attribution-lint\.mjs check --write-baseline`/.test(trimPart) && /attribution-lint\.baseline\.json/.test(trimPart),
   's16 стрижка (CK5.6): первое закрытие без базы — навык велит записать унаследованный долг линта авторства один раз и назвать файл базы', trimPart.slice(-900) || 'trim/door headings not found');
