...
const CLASSES = [
  ...
  },
  {
    ...
    forms: [
      ...
      // «фактический результат» — название раздела отчёта тестировщика (ISTQB; шаблон C `/report-bug`, линтер `bug` принимает
      // именно его), не намёк на закулисье: ноты 2.8, назвавшие раздел, краснели, а синоним сделал бы ноту неверной (§7Б, сессия 75)
      /(?<![А-Яа-яЁё-])фактическ(?:и|ое|ий|ая|ого|ой)(?![А-Яа-яЁё-])(?!\s+результат)/giu,
      /\bin\s+reality\b/gi,
  ...
  },
  {
    id: 'NBSP',
    level: HARD,
    name: 'обычный пробел между числом и единицей — перенос строки отрывает единицу от величины',
    fix: 'поставь неразрывный пробел U+00A0 между числом и единицей: «5 $», «40 часов»',
    // Слово владельца 2026-08-09 со снимком экрана, где «по 5 $» разорвано переносом: «нужен
    // неразрывный пробел между цифрами и единицей измерения» · «перепроверить ВЕЗДЕ».
    // Класс механизирует правило, которое дважды за вечер жило одноразовым скриптом и дважды
    // молчало (EXP-0082): после `$` НЕТ границы слова, а `\b` в JavaScript слеп к кириллице —
    // поэтому здесь границы заданы явно, а символьные единицы вынесены в отдельную форму.
    forms: [
      /(\d) (?=[$%€₽]|кВт·ч|кВт)/gu,
      new RegExp('(\\d) (?=(?:' + [
        'часов', 'часа', 'час', 'минут', 'минуты', 'суток', 'сутки', 'дней', 'дня',
        'слов', 'слова', 'строк', 'строки', 'страниц', 'страницы', 'романов', 'романа',
        'токенов', 'токена', 'человеко-часов', 'гамбургеров', 'коммитов', 'коммита',
        'сообщений', 'сообщения', 'чатов', 'чата', 'знаков', 'инженеров', 'книг',
        'hours', 'hour', 'minutes', 'words', 'lines', 'pages', 'novels', 'tokens',
        'man-hours', 'person-hours', 'messages', 'commits', 'engineers', 'books', 'kWh',
      ].join('|') + ')(?![\\p{L}\\p{N}-]))', 'gu'),
    ],
  },
];
...
function checkNotesVsReadme(notesText, readmeText) {
  ...
  // Пара живёт, пока живут ОБЕ стороны. Слово владельца (ДЗ-06, волна №3, 2026-08-21 18:31
  // +03:00: «по 2.3 не пишем») сняло блок метрик из README целиком — сверять числа нот стало
  // не с чем, и красный оси превратился бы в ложный. Якорь стороны — заголовок блока метрик:
  // вернётся блок в README любой будущей версии → ось оживает сама, без правки этого файла.
  if (!/Metrics of KAIF|Метрики версии KAIF/.test(readmeText)) return hits;
...
function selftest() {
  ...
  const clean = [
    ...
    '|---|---|',
    // Неразрывный пробел здесь НАМЕРЕННЫЙ: фикстура обязана проходить и класс NBSP тоже.
  ...
  const mutations = [
    ...
    // Обе ветки класса NBSP — ровно те две, что молча не работали в поле (EXP-0082):
    // символьная единица (после `$` нет границы слова) и кириллическая (в JS `\b` слеп к
    // не-ASCII). Первая мутация — дословная строка со снимка владельца.
    ['NBSP', clean + '\n\n| Фактически заплачено | 11 гамбургеров по 5 $ |'],
    ['NBSP', clean + '\n\nАктивной работы вышло 40 часов за трое суток.'],
  ];

  // 2b. ОХВАТ СЕЛФТЕСТА ВЫЧИСЛЯЕТСЯ, А НЕ ОБЪЯВЛЯЕТСЯ: прогон печатает «каждый класс доказан», и
  // это заявление обязано быть проверяемым. Класс, добавленный без мутации, прежде проходил
  // молча — селфтест зеленел, а новая проверка не была доказана ни разу (`TESTING_FRAMEWORK.md`:
  // проверка, которая ни разу не падала, ничего не доказывает).
  const covered = new Set(mutations.map(([id]) => id));
  const uncovered = CLASSES.map((c) => c.id).filter((id) => !covered.has(id));

  let bad = 0;

  // 0. ОХВАТ: у каждого построчного класса есть мутация.
  console.log(`${uncovered.length ? '❌' : '✅'} охват селфтеста: классов ${CLASSES.length}, ` +
              `без мутации ${uncovered.length}${uncovered.length ? ` — ${uncovered.join(', ')}` : ''}`);
  if (uncovered.length) bad++;
  ...
  // 2e. Граница класса BACKSTAGE: название раздела отчёта тестировщика «фактический результат» молчит (красный случай
  // «Заплачено владельцем фактически …» выше остаётся красным — исключение узкое).
  const qaTerm = clean + '\n\nВ отчёте четыре раздела — описание, шаги воспроизведения, ожидаемый результат и фактический результат.';
  const qaHits = checkText(qaTerm).filter((h) => h.id === 'BACKSTAGE');
  const qaOk = qaHits.length === 0;
  console.log(`${qaOk ? '✅' : '❌'} граница BACKSTAGE: «фактический результат» (раздел отчёта тестировщика) молчит (находок ${qaHits.length}, ждали 0)`);
  if (!qaOk) bad++;
  ...
  const readmeFix = 'Метрики версии KAIF 9.9. Слов прозы **316 764**, символов **2 149 895**, романов **17 919**.';
  ...
  if (!driftOk) bad++;
  if (!driftBad) bad++;
  // Пара мертва по слову владельца (README без блока метрик) — ось обязана МОЛЧАТЬ, а не
  // краснить единственный живой замер нот; воскрешение — возврат якоря в README (кейс выше).
  const driftDead = checkNotesVsReadme(notesBad, 'README без блока метрик версии.').length === 0;
