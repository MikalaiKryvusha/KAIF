...
// bugs/81 — ТОТ ЖЕ урок, провалившийся во ВТОРОЙ раз, и потому переписанный формой. Примета
// по-прежнему делила с парсером слепое пятно, только уже другое: обе стороны требовали, чтобы
// после буквы СРАЗУ шёл разделитель клетки, и обе не видели живую форму «| **A** (рекомендация
// агента) |». Согласие двух слепых читалось как доказательство, и в опубликованном №016 владелец
// увидел три варианта из четырёх. Теперь примета считает то, что видит ЧЕЛОВЕК: клетка, ОТКРЫТАЯ
// намеренной меткой варианта (жирная буква или буква с точкой/скобкой), чем бы она дальше ни
// продолжалась. Обычная строка данных («| Antigravity | …») меткой не открывается и не считается.
// bugs/104 — тот же урок В ТРЕТИЙ раз (№019): жирная метка законна и с пометкой в скобках ДО
...
function makeFixtureRoot(root = tempRoot('verify-contour')) {
  ...
  writeFileSync(join(root, 'interviews', 'interview_101_fixture.md'), [
    ...
    '',
    // bugs/104: табличная развилка с ОБЕИМИ формами пометки — внутри жирного (№019) и после
    // жирного (№016) — плюс голая буква. Каждый полный прогон наблюдает СТРАНИЦЕЙ, что все три
    // строки стали радиокнопками: класс «строка выглядит вариантом для человека, но не для
    // парсера» чинился трижды, и формы, однажды потерянные, отныне живут в фикстуре постоянно.
    '### Q3. Табличная развилка с пометками?',
    '',
    '| Вариант | Что означает | Цена |',
    '|---|---|---|',
    '| **A (Рекомендация)** | пометка внутри жирного | а |',
    '| **B** (пометка) | пометка после жирного | б |',
    '| **C** | голая буква | в |',
    '',
    '**Answer:**',
    '',
  ...
  try {
    ...
    for (const scheme of ['light', 'dark']) {
      for (const width of [1100, 500]) {
        ...
        // всего 7 радио (2 активных Q1 + 2 выключенных Q2 + 3 активных табличных Q3 — обе
        // формы пометки и голая буква, bugs/104), активных — 5.
 ...
 * ритуале. Дешёвая половина обязана гоняться часто (`/end-chat-soft`), дорогая — редко (`/release`).
...
// Детектор КЛАССА (bugs/69, второй пункт плана): фикс убирает пять зашитых обращений, а страж
// не даёт им вернуться. Примета — литеральное имя живого интервью в ИСПОЛНЯЕМОЙ строке этого же
// файла: комментарии изъяты явно, потому что дефект объясняют именно ими (запрет упоминать имя в
// объяснении сделал бы невозможным разбор урока следующей сессией).
// Проверка смотрит на СЕБЯ — это законно и дёшево: класс живёт ровно в одном файле, и вынесение
// его наружу породило бы второй список, расходящийся с первым (тот самый дрейф двух списков).
// Примета сужена ровно до класса: имя считается зашитым, только если так названный документ
// РЕАЛЬНО ЖИВЁТ в репозитории. Первая редакция краснела на именах ФИКСТУРЫ
// (`interview_101_fixture.md`) — а фикстура и есть законный способ гонять правила по норме G5,
// и запретить её имена значило бы запретить сам приём. Существование на диске отличает живой
// документ от синтетического без всяких списков.
function selfHardwiredLiveNames() {
  const src = readFileSync(fileURLToPath(import.meta.url), 'utf8');
  const out = [];
  for (const line of src.split('\n')) {
    if (/^\s*(\/\/|\*|\/\*)/.test(line)) continue;         // комментарии — там имя объясняет дефект
    // Точка в классе символов обязательна: живые имена несут версию (`interview_007_kaif_2.2.md`),
    // и без неё детектор был слеп ровно на том, ради чего написан. Поймано мутацией, не чтением.
    for (const m of line.matchAll(/interview_[0-9A-Za-z_.]+\.md/g)) {
      if (existsSync(join(ROOT, 'interviews', m[0]))) { out.push(line.trim()); break; }
    }
  }
  return out;
}
...
function etalonBlock() {
  ...
  const hardwired = selfHardwiredLiveNames();
  check('в исполняемых строках прогона НЕТ зашитых имён живых интервью (bugs/69: имя и число берутся из эталона)',
    hardwired.length === 0, hardwired.slice(0, 3).map((l) => l.trim()).join(' | '));
